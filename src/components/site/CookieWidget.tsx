...
export function CookieWidget() {
  ...
  return (
    <button
      ...
      aria-label={label}
      title={label}
      className="fixed bottom-24 left-4 z-[60] inline-flex h-11 w-11 items-center justify-center rounded-full border border-border bg-card text-foreground shadow-lg shadow-black/20 transition-all hover:scale-105 hover:text-brand md:bottom-6 md:left-6 md:h-12 md:w-12"
    >
      <Cookie size={20} />
    </button>
  );
